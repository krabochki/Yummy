...
export class ControlDashboardComponent implements OnInit, OnDestroy {
  ...
  achievementsArVariants=['достижение','достижения','достижений']
  ...
  usersArVariants = ['пользователь', 'пользователя', 'пользователей'];
  ...
  private initAllData(): void {
    ...
    if (this.currentUser.role === 'admin') {
      ...
      count$.push(this.getExpiredUsersCount());
    }
    this.subscriptions.add(
      forkJoin(count$).pipe(takeUntil(this.destroyed$)).subscribe(),
    );
  }
  ...
  getExpiredUsersCount() {
    return this.authService.getExpiredUsersCount().pipe(
      tap((res: any) => {
        this.expiredUsersLength = res[0].count;
        this.cd.markForCheck();
      }),
    );
  }
  expiredUsersLength = -1;
