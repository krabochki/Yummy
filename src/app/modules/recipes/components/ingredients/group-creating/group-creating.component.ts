...
export class GroupCreatingComponent implements OnInit, OnDestroy {
  ...
  private PUTGroup(group: IGroup) {
    ...
    const putCategory$ = this.groupService.updateGroup(group).pipe(
      catchError((response: any) => {
        if (response.error.info == 'NAME_EXISTS') {
          ...
          this.throwErrorModal('Произошла ошибка при попытке обновить группу.');
        }
        return EMPTY;
      }),
    );
      ...
      .pipe(
        ...
        catchError((response: any) => {
          if (response.error.info == 'NAME_EXISTS') {
            ...
            this.throwErrorModal(
              'Произошла ошибка при попытке добавить группу.',
            );
          }
          return EMPTY;
        }),
    ...
    if (this.currentUser.id) {
      if (
        ...
        if (this.edit) {
          ...
          notify = this.notifyService.buildNotification(
            'Раздел добавлен',
            `Вы успешно добавили новую группу ингредиентов «${this.savedGroup.name}»`,
            'success',
            'ingredient',
            `/groups/list/${this.savedGroup.id}`,
          );
