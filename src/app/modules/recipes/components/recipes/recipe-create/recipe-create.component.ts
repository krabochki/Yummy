      ...
      concatMap((res) => {
        if (res.image) {
          ...
          return of(null);
            ...
            combineLatest([categories$, instructions$, ingredients$]).subscribe(
              () => {
                image$
                  .pipe(
                    finalize(() => {
      ...
      .pipe(
        ...
        concatMap((response: any) => {
          ...
          return combineLatest(
            ingredients$.length ? ingredients$ : of(null),
          ).pipe(
            concatMap(() =>
  ...
  PUTRecipe(recipe: IRecipe) {
    ...
    putRecipe$
      .pipe(
        ...
        concatMap(() =>
          combineLatest(ingredients$.insert).pipe(
            catchError(() => {
              this.throwErrorModal(
        ...
        concatMap(() =>
          combineLatest(instructions$.delete.length? instructions$.delete : of(null)).pipe(
            catchError(() => {
              this.throwErrorModal(
